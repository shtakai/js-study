  ...
  test('requesting a group rate quote from the hood river tour page should populate the referrer field', function(done) {
    ...
    browser.visit(referrer, function() {
      browser.clickLink('.requestGroupRate', function(){
				assert(browser.field('referrer').value === 'referrer'); // This will have to pass but fails... https://github.com/EthanRBrown/web-development-with-node-and-express/issues/50
				done();
			});
    });
  });

  test('requesting a group rate from the oregon coast tour page shoud populate the refferer field', function(done) {
